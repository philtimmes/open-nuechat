...
Changes:
- NC-0.8.0.10: **Fresh Install Fix** - (1) `seed_default_settings()` populates `system_settings` from `SETTING_DEFAULTS` on startup if empty. (2) Fresh installs now detected properly and start at `NC-0.0.0` so all migrations run. Detection: no version row + empty system_settings = fresh install.
