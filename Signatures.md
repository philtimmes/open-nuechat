...
Changes:
- NC-0.8.0.10: **Fresh Install Settings Seeding** - New `seed_default_settings()` function in `main.py` lifespan populates `system_settings` table from `SETTING_DEFAULTS` on first startup if empty. Fixes "Failed to load settings" error in Admin Panel on fresh installs.
